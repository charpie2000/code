...
//changes the color of the clicked circle in the player's control area.
function changeColor(obj) {
    ...
    switch (innerText) {       //moves the circle's color to the next color.
        case 0:     //0 represents green. The next color is blue, so if the inner text is 0, change the color of the circle to blue.
            obj.style.backgroundColor = '#6FC6F7'; // Green
            obj.style.color = '#6FC6F7';
            obj.textContent = 1;
            break;
        case 1:
            obj.style.backgroundColor = '#ED6B7F'; // Blue
            obj.style.color = '#ED6B7F';
            obj.textContent = 2;
            break;
        case 2:
            obj.style.backgroundColor = '#FADD5F'; // Red
            obj.style.color = '#FADD5F';
            obj.textContent = 3;
            break;
        case 3:
            obj.style.backgroundColor = '#000000'; // Yellow
            obj.style.color = '#000000';
            obj.textContent = 4;
            break;
        case 4:
            obj.style.backgroundColor = '#C866F2'; // Black
            obj.style.color = '#C866F2';
            obj.textContent = 5;
            break;
        case 5:
            obj.style.backgroundColor = '#FFA500'; // Purple
            obj.style.color = '#FFA500';
            obj.textContent = 6;
            break;
        case 6:
            obj.style.backgroundColor = '#8D6E63'; // Orange
            obj.style.color = '#008080';
            obj.textContent = 7;
            break;
        case 7:
            obj.style.backgroundColor = '#67E06F'; // Teal
	...
	for (i = 0; i < 4; i++) {							//for loop adds a new circle to the string for each element in the player's array.
		switch (plArr[i]) {			//like above, 0 = green, 1 = blue, etc.
			...
			case 3:
				...
				break;
			case 4:
				newCircles += "<div class = 'symbols' style = 'background-color: #000000;'></div>";
				break;
			case 5:
				newCircles += "<div class = 'symbols' style = 'background-color: #C866F2;'></div>";
				break;
            case 6:
				newCircles += "<div class = 'symbols' style = 'background-color: #FFA500;'></div>";
				break;
			case 7:
				newCircles += "<div class = 'symbols' style = 'background-color: #8D6E63;'></div>";
				break;    
...
function checkArrays() {
	...
	if (black == 4) {										//if there are 4 correct colors in the correct position, the code is correct.
		...
		printGuess(plArr, black, white);			//print the guess to the game area.
         if (guesses >= 8) {                        // Check if the player has exceeded xx attempts
