...
function winner(guesses) {		//function brings up a popup, tells the user their score, then gives them the option to refresh the page.
	...
	var textStuff = "<center><h1>Bravo!</h1>Vous avez trouvé<br>le code maître en ";	//variable for the text that goes inside the popup.
	textStuff += guesses;
	textStuff += " coups.<br />"
    ...
    textStuff += "<button id = 'codeBtn' onclick = 'copyMessage();'>Copy</button></center>";
...
function copyMessage() {
    ...
    let message = `🟢🔵🔴🟡⚫️🟣🟠🟤\n`;
    message += `J'ai trouvé le CodeMaître en ${guesses} coups!\n`;
    message += `\nJoué au CodeMaître ici:\nhttps://charpie2000.github.io/cm`;
    message += `\n🟢🔵🔴🟡⚫️🟣🟠🟤\n`;
